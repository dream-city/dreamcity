...
contract InvestorStorage is Ownable {
    ...
    uint256 public maxCountStep = 25;
    ...
    function fixArrayPaidTokenLastDay() internal returns (uint256 countElem) {
        ...
        if (lengthArray > 1) {
            while (countElem < lengthArray) {
                ...
                if ((maxCountStep < lengthArray) || (numberTodayDay > currentNumberDay)) {
            ...
            while (step < amountLastToken.add(1)) {
                if (lastNumberInvestor > 0) {
                    ...
                    if (step == 0) {
                        ...
                        if (currInvestor.send(ethLastInvestor)) {
                        ...
                        if (currInvestor.send(ethCurrentInvestor)) {
...
contract DreamCity is Ownable, HouseStorage {
    ...
    function setMaxCountStep(uint256 _number) external onlyOwner {
        require(_number > 0 && _number < 40);
        maxCountStep = _number;
    }
    ...
    function setMinNumberSalesTokens(uint256 _number) external onlyOwner {
        require(_number > 0 && _number <= maxCountStep);
